...
from internal.workflows.tokenization.contract_consts import EVENT_FRONTEND
...
def create_workflow_payment(workflow_data):
    ...
    sender_balance = verify_is_party_free_balance_enough(
        workflow_data["sender_party"], int(workflow_data.get("amount")), USDZ)
...
def get_workflows():
    ...
    print(workflow_status_history_models)
    if data.get("page"):
...
def get_workflow_data(workflow_id):
    ...
    if workflow_id:
        ...
        response_list = []
        for row in tb.select().where(tb.workflow_id == workflow_id):
            row_data = row.to_json()
            print(row_data)
            row_data['created_at'] = row_data['created_at'].strftime('%Y-%m-%d %H:%M:%S')
            log_data = col.find_one({'workflow_id': row_data['workflow_id']})
            row_data['workflow_log'] = str(log_data)
...
if __name__ == '__main__':
    mongo_client = get_mongo_client()
    col = mongo_client.col_workflow_log
    res = col.find({'workflow_id': "2419aa84-3969-4968-959c-c96522295ad7"})
    a = col.find({'workflow_id':'1'})
    for i in a:
        print(i)
    print([res])
