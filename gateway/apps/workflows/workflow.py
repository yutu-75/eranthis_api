...
from peewee import FieldAccessor, operator, fn, Value

from gateway.apps.common.workflow_ws import get_history_workflow_data
...
def get_history_workflows():
    ...
    where_list = []
    if network:
        where_list.append(WorkflowStatusHistory.networks.contains(network.split(',')))
    ...
    if status == "Success":
        where_list.append(WorkflowStatusHistory.status_code >= 10000)
    elif status == "Failed":
        where_list.append(WorkflowStatusHistory.status_code >= 50000)
    ...
    if where_list:
        workflow_status_history_models = WorkflowStatusHistory.select(
            ...
            fn.CONCAT(WorkflowStatusHistory.networks).alias("networks"),
            ...
            WorkflowStatusHistory.workflow_id,
            WorkflowStatusHistory.workflow_name,
            fn.CONCAT(WorkflowStatusHistory.networks),
        ).order_by(- Value(WorkflowStatusHistory.created_at).alias("created_at"))
    else:
        workflow_status_history_models = WorkflowStatusHistory.select(
            WorkflowStatusHistory.workflow_id,
            WorkflowStatusHistory.workflow_name,
    ...
    total = workflow_status_history_models.count()

    if data.get("page"):
        if data.get("page"):
            workflow_status_history_models = workflow_status_history_models.paginate(
                page=int(data.get("page")), paginate_by=int(data.get("page_size", 20)))
...
@bp_workflow.route("/all_workflow", methods=['GET'])
@LogDecorator()
def get_all_workflow_data():
    """
    根据workflow_id获取workflow数据
    :workflow_id int workflow_id
    :return:
    """
    response_list = get_history_workflow_data()
